...
const stats = calcMesStats(transacoes, [], contas, mes);
assert(approx(stats.totalReceitasMes, 4000), "receitas sem transferência");
assert(approx(stats.totalGastosMes, 35), "gastos sem transferência e sem CC de abril");
assert(approx(stats.saldoMes, 3965), "saldo = receitas - gastos");
assert(stats.totalTransacoes === 6, "conta todas transações");

section("Cartão: compra após fechamento vai para mês seguinte");
const abril = calcMesStats(transacoes, [], contas, "2025-04");
